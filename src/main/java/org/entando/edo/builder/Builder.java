...
public class Builder {
	...
	public void build(EdoBean bean) throws Throwable {
	...
	private void saveReport(Render render, Map<String, Object> contextElements, EdoBean bean) {
		...
		try {
			...
			file = new File(bean.getBaseDir() + File.separator + filename);
	...
	private void writeJavaModel(Render render, Map<String, Object> contextElements, EdoBean bean) {
		try {
			...
			this.writeFile(bean.getBaseDir(), pojopath, pojo);
			...
			this.writeFile(bean.getBaseDir(), managerIterfacePath, managerInterface);
			...
			this.writeFile(bean.getBaseDir(), observerPath, observer);
	...
	private void writeAPI(Render render, Map<String, Object> contextElements, EdoBean bean) {
		try {
			...
			this.writeFile(bean.getBaseDir(), apiBeanPath, apiBean);
			...
			this.writeFile(bean.getBaseDir(), apiResponsePath, apiResponse);
			...
			this.writeFile(bean.getBaseDir(), apiResponseResultPath, apiResponseResult);
			...
			this.writeFile(bean.getBaseDir(), apiListResponsePath, apiListResponse);
			...
			this.writeFile(bean.getBaseDir(), apiListResponseResultPath, apiListResponseResult);
			...
			this.writeFile(bean.getBaseDir(), apiMethodsPath, apiMethods);
	...
	private void writeSpringModelXml(Render render, Map<String, Object> contextElements, EdoBean bean) {
		try {
			...
			this.writeFile(bean.getBaseDir(), stringPath, xmlContent);
		} catch (Throwable t) {
			logger.error("error!", t);
		}
	}
	...
	private void writeEntantoInit(Render render, Map<String, Object> contextElements, EdoBean bean) {
		try {
			...
			this.writeFile(bean.getBaseDir(), pojoPath, javaContent);
			...
			this.writeFile(bean.getBaseDir(), componentPath, xmlContent);
			...
			this.writeFile(bean.getBaseDir(), sqlServPath, sqlServContent);
			//PROD SQL PORT
			...
			this.writeFile(bean.getBaseDir(), sqlPortPath, sqlPortContent);
			...
			this.writeFile(bean.getBaseDir(), sqlTestServPath, sqlTestServContent);
			//TEST SQL PORT
			...
			this.writeFile(bean.getBaseDir(), sqlTestPortPath, sqlTestPortContent);
	...
	private void writeAction(Render render,	Map<String, Object> contextElements, EdoBean bean) {
		try {
			...
			this.writeFile(bean.getBaseDir(), actionFilePath, actionJava);
			...
			this.writeFile(bean.getBaseDir() , finderActionFilePath, finderActionJava);
			...
			this.writeFile(bean.getBaseDir() , actionXmlFilePath, actionXmlContent);
			...
			if (bean.needsActionStrutsConversionProperties()) {
				...
				this.writeFile(bean.getBaseDir() , propFilePath, actionConvPropertiesContent);
			}
			if (bean.needsFinderActionStrutsConversionProperties()) {
				...
				this.writeFile(bean.getBaseDir() , convFilePath, actionConvPropertiesContent);				
			}
			...
			if (bean.needsStrutsActionValidation()) {
				...
				this.writeFile(bean.getBaseDir() , validationFilePath, actionValidationContent);				
			}
			...
			this.writeFile(bean.getBaseDir() , actionPropertiesItFilePath, actionPropertiesItContent);
			...
			this.writeFile(bean.getBaseDir() , actionPropertiesEnFilePath, actionPropertiesEnContent);
			...
			this.writeFile(bean.getBaseDir() , strutsPluginFilePath, actionStrutsPluginContent);
			...
			this.writeFile(bean.getBaseDir() , shortcutFilePath, shortcutContent);
			...
			this.writeFile(bean.getBaseDir() , tilesFilePath, actionTilesContent);
			...
			this.writeFile(bean.getBaseDir() , springFilePath, actionSpringContent);
			...
			if (bean.isPlugin()) {
				...
				this.writeFile(bean.getBaseDir() , globalMessagesPropertiesFilePath, globalMessagesPropertiesContent);
	...
	private void writeWidget(Render render,	Map<String, Object> contextElements, EdoBean bean) {
		try {
			...
			this.writeFile(bean.getBaseDir(), specialWidgetActionFilePath, specialWidgetActionJava);
			...
			this.writeFile(bean.getBaseDir() , specialWidgetActionXmlFilePath, specialWidgetActionXmlContent);
			...
			this.writeFile(bean.getBaseDir() , specialWidgetActionPropertiesItFilePath, specialWidgetActionPropertiesItContent);
			...
			this.writeFile(bean.getBaseDir() , specialWidgetActionPropertiesEnFilePath, specialWidgetActionPropertiesEnContent);
			...
			this.writeFile(bean.getBaseDir(), specialWidgetJspFilePath, specialWidgetJsp);
			...
			this.writeFile(bean.getBaseDir(), listTagFilePath, listTagJava);
			...
			this.writeFile(bean.getBaseDir(), singleTagFilePath, singleTagJava);
			...
			this.writeFile(bean.getBaseDir(), apsTldFilePath, apsTld);
			...
			this.writeFile(bean.getBaseDir(), widgetJspFilePath, widgetJsp);
	...
	private void writeInternalServlet(Render render, Map<String, Object> contextElements, EdoBean bean) {
		...
		try {
			...
			this.writeFile(bean.getBaseDir() , internalServletXmlFilePath, internalServletXmlContent);
			...
			this.writeFile(bean.getBaseDir() , internalServletActionFilePath, internalServletActionContent);
			...
			this.writeFile(bean.getBaseDir() , internalServletFinderActionFilePath, internalServletFinderActionContent);
			...
			this.writeFile(bean.getBaseDir() , internalServletJspListFilePath, internalServletJspListFileContent);
			...
			this.writeFile(bean.getBaseDir() , internalServletJspEntryFilePath, internalServletJspEntryFileContent);
			...
			this.writeFile(bean.getBaseDir() , internalServletJspTrashFilePath, internalServletJspTrashFileContent);
			...
			this.writeFile(bean.getBaseDir() , internalServletJspErrorFilePath, internalServletJspErrorFileContent);
		} catch (Throwable t) {
			logger.error("error!", t);
		}
	}
	...
	private void writeJspAction(Render render,	Map<String, Object> contextElements, EdoBean bean) {
		try {
			...
			this.writeFile(bean.getBaseDir(), entryFilePath, actionJsp);
			...
			this.writeFile(bean.getBaseDir(), entryExtrasFilePath, entryExtrasJsp);
			...
			this.writeFile(bean.getBaseDir(), listFilePath, actionListJsp);
			...
			this.writeFile(bean.getBaseDir(), listExtrasFilePath, listExtrasJsp);
			...
			this.writeFile(bean.getBaseDir(), trashFilePath, actionTrashJsp);
			...
			this.writeFile(bean.getBaseDir(), trashExtrasFilePath, trashExtrasJsp);
			...
			if (bean.isPlugin()) {
				...
				this.writeFile(bean.getBaseDir(), submenuFilePath, submenuJsp);
			}
	...
	private void writeTestService(Render render, Map<String, Object> contextElements, EdoBean bean) {
		try {
			...
			this.writeFile(bean.getBaseDir(), configTestUtil, configTestUtilJava);
			...
			this.writeFile(bean.getBaseDir(), baseTestFilePath, baseTestJava);
			...
			this.writeFile(bean.getBaseDir(), managerTestFilePath, managerTestJava);
	...
	private void writeTestAction(Render render,	Map<String, Object> contextElements, EdoBean bean) {
		try {
			...
			this.writeFile(bean.getBaseDir(), baseTestactionFilePath, baseTestActionJava);
